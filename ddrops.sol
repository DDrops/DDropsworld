...
contract Ddrops is Context, Ownable, IERC20  {
    ...
    function setMaxTrxAmount (uint newMaxTrxAmount) external onlyOwner returns (bool){
        _maxTrxAmount = newMaxTrxAmount;
        return true;
    }
    
    function setAmountToLiquify(uint newAmountToLiquify) external onlyOwner returns (bool){
        _amountToLiquify = newAmountToLiquify;
        return true;
    }
    
    function setContributionFee(uint newContributionFee) external onlyOwner returns (bool){
        contributionFee = newContributionFee;
        return true;
    }
    
    function setSwapAndLiquifyEnabled(bool newValue) external onlyOwner returns (bool){
        swapAndLiquifyEnabled = newValue;
        
        emit SwapAndLiquifiedUpdate(newValue);
        return true;
    }
    
    function setDevelopementWallet(address newDevelopementWallet) external onlyOwner returns (bool){
        developementWalletAddress = newDevelopementWallet;
        return true;
    }
    
     function setTreasureWallet(address newTreasureWallet) external onlyOwner returns (bool){
        treasureWalletAddress = newTreasureWallet;
        return true;
    }
    ...
    constructor() {
        ...
        _balances[_msgSender()] = 3375 * 10 ** 5 * 10 ** 9;     
        
        contributionFee = 0;

        treasureWalletAddress = 0x6260845ea07Bfe5A8B5463b83B1d3B2c53D2dCE8;
        developementWalletAddress = 0xE1032FF98B26b634393612E59819DcEe39F8c4b8;
    ...
    function _transfer(address from, address to, uint amount) private{
        ...
        require(amount <= _balances[from], "Insufficient funds");
         ...
         if(from != owner() && to != owner())
            require(amount <= _maxTrxAmount, "Transfer amount exceeds the maxTrxAmount.");
    ...
    function _transferToken(address _from, address _to, uint _amount, bool takeFee) private noReEntry {
        if(!takeFee)
            removeAllFee();
            
        
        uint _takeContribution = calculateAbsoluteContributionFee(_amount);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private noReEntry {
        ...
        sendValue(payable(developementWalletAddress), _developementWalletAmount);
        sendValue(payable(treasureWalletAddress), _treasureWalletAmount);
    }
    ...
    function removeAllFee() private {
        if(contributionFee == 0) return;
        
        _previousContributionFee = contributionFee;
        
        contributionFee = 0;
    }
    
    function restoreAllFee() private {
        contributionFee = _previousContributionFee;
    }
    
    function calculateAbsoluteContributionFee(uint amount) internal view returns (uint){
        return amount.mul(contributionFee).div(10**2);
    }
